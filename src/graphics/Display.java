		...
		public void paintComponent(Graphics g) {
			...
			for (Object3d object: objectList) {
				...
				for (Triangle face: objectGeometry) {
					...
					Vector2 p1 = Vector.toScreenSpace(v1, eye);
					Vector2 p2 = Vector.toScreenSpace(v2, eye);
					Vector2 p3 = Vector.toScreenSpace(v3, eye);
					...
					// Vertices in rasterized space
					Vector2 n1 = Vector.toNormalSpace(p1, width, height);
					Vector2 n2 = Vector.toNormalSpace(p2, width, height);
					Vector2 n3 = Vector.toNormalSpace(p3, width, height);
					
					// The dimensions of an imaginary rectangle around the triangle in raster space
                    int minX = (int) Math.ceil(Math.min(n1.x, Math.min(n2.x, n3.x)));
                    int maxX = (int) Math.min(width - 1,
                            Math.floor(Math.max(n1.x, Math.max(n2.x, n3.x))));
                    int minY = (int) Math.ceil(Math.min(n1.y, Math.min(n2.y, n3.y)));
					...
					for(int x = minX; x <= maxX; x++) {
						for(int y = minY; y <= maxY; y++) {
							...
							boolean hasColour = Vector2.isInTriangle(n1, n2, n3, pixelPosition);
							if(hasColour) {
